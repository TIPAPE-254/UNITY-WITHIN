...
import { pool, testConnection, initializeDatabase, isDatabaseAvailable } from './db.js';
import { sendResetEmail, sendVolunteerInvite, sendTherapistInvite, sendEmail } from './brevoMailer.js';
import { detectEmotionWithKenyanLayer, getKenyanCrisisBridgeResponse } from './sheng.js';
import { aiConfig, AI_UNREACHABLE_MESSAGE, getAiProviderStatus } from './config.js';
import { buildClerkMiddleware, requireStrictClerkSession, syncClerkAppUser } from './clerk.js';
...
const logAiProviderStatus = () => {
    ...
    console.log('🤖 AI Provider Status (from runtime env / Azure App Settings):', status);
};
...
const BUDDIE_MEMORY_WINDOW = parseInt(process.env.BUDDIE_MEMORY_WINDOW || '6', 10);
const BUDDIE_MEMORY_MAX = parseInt(process.env.BUDDIE_MEMORY_MAX || '60', 10);

// Buddie configuration
const BUDDIE_DIALOG_DATA_PATH = process.env.BUDDIE_DIALOG_DATA_PATH || './data/dailydialog_examples.json';
const BUDDIE_COUNSELING_DATA_PATH = process.env.BUDDIE_COUNSELING_DATA_PATH || './data/mental_health_counseling_examples.json';
const BUDDIE_LIVESIYAM_DATA_PATH = process.env.BUDDIE_LIVESIYAM_DATA_PATH || './data/livesiyam_examples.json';
const BUDDIE_DIALOG_FEWSHOT_COUNT = parseInt(process.env.BUDDIE_DIALOG_FEWSHOT_COUNT || '2', 10);
const BUDDIE_REPLY_MAX_TOKENS = parseInt(process.env.BUDDIE_REPLY_MAX_TOKENS || '85', 10);
...
const BUDDIE_HF_DATASETS = (process.env.BUDDIE_HF_DATASETS || `${BUDDIE_HF_DATASET}:${BUDDIE_HF_CONFIG},facebook/empathetic_dialogues:default`)
    .split(',')
    .map((entry) => entry.trim())
    .filter(Boolean);
const BUDDIE_HF_ROWS_LIMIT = Math.max(10, parseInt(process.env.BUDDIE_HF_ROWS_LIMIT || '120', 10));
const BUDDIE_EMPATHY_BANK_PATH = process.env.BUDDIE_EMPATHY_BANK_PATH || './data/unityEmpathyBank.json';
...
const validateProductionConfig = () => {
    if (isProduction) {
        ...
        const missing = required.filter((k) => {
            ...
            return !(direct && String(direct).trim()) && !(appSetting && String(appSetting).trim());
        });
        if (missing.length) {
...
if (!process.env.ADMIN_EMAIL && process.env.ENCRYPTED_ADMIN_EMAIL) {
    try {
        ...
        console.log('✅ Admin email decrypted and loaded from ENCRYPTED_ADMIN_EMAIL');
    } catch (error) {
        console.error('❌ Failed to decrypt ENCRYPTED_ADMIN_EMAIL:', error.message);
        console.warn('⚠️ Admin authentication will be disabled. Set ADMIN_EMAIL directly or provide valid ENCRYPTED_ADMIN_EMAIL + ENCRYPTION_KEY');
    }
}
...
app.get('/api/user/is-admin', (req, res) => {
    ...
    const userEmail = (req.headers['x-user-email'] || req.user?.email || '').toString().toLowerCase().trim();
    ...
    try {
        ...
        if (!row) {
            return res.status(401).json({
                ...
                message: 'Unable to resolve user from database. Provide a valid user id.',
            });
        }
...
const setSharedUserCookie = (req, res, user) => {
    ...
    const secure = req.secure || String(req.headers['x-forwarded-proto'] || '').toLowerCase().includes('https');
...
const getEventsAuthSecret = () => {
    const explicitSecret = process.env.EVENTS_AUTH_SECRET || process.env.APPSETTING_EVENTS_AUTH_SECRET;
    if (explicitSecret && String(explicitSecret).trim()) {
        return String(explicitSecret).trim();
    }
    const clerkSecret = process.env.CLERK_SECRET_KEY || process.env.APPSETTING_CLERK_SECRET_KEY;
...
const verifyEventsAuthTicket = (ticket) => {
    try {
        const secret = getEventsAuthSecret();
        if (!secret || !ticket || typeof ticket !== 'string' || !ticket.includes('.')) {
            return null;
        }
...
const resolveUserFromEventsTicket = async (ticketPayload) => {
    ...
    if (ticketPayload.email && String(user.email || '').toLowerCase() !== String(ticketPayload.email).toLowerCase()) {
        return null;
    }

    if (ticketPayload.clerkUserId && String(user.clerk_user_id || '') !== String(ticketPayload.clerkUserId)) {
        return null;
    }
...
const resolveEventsIdentity = async (req) => {
    ...
    if (parsedTicket) {
        try {
            ...
            if (resolvedUser) {
                console.log(`[RSVP Auth] Identified via events ticket for user ${resolvedUser.email}`);
                return resolvedUser;
            }
        } catch (error) {
    ...
    try {
        ...
        if (userRows?.length) {
            console.log(`[RSVP Auth] Identified via Unity headers for user ${userRows[0].email}`);
            return {
                id: userRows[0].id,
                email: userRows[0].email,
                role: (userRows[0].email === SUPER_ADMIN_EMAIL) ? 'admin' : (userRows[0].role || 'user'),
    ...
    try {
        ...
        if (sharedPayload) {
            ...
            if (rows?.length) {
                console.log(`[RSVP Auth] Identified via shared cookie for user ${rows[0].email}`);
                return {
                    id: rows[0].id,
                    email: rows[0].email,
                    role: (rows[0].email === SUPER_ADMIN_EMAIL) ? 'admin' : (rows[0].role || 'user'),
...
const requireEventsRsvpIdentity = async (req, res, next) => {
    ...
    console.log('[RSVP Auth] Missing valid identity (ticket, headers, or cookie)');
    return res.status(401).json({
        success: false,
        error: 'Authentication required',
...
app.post('/api/auth/clerk/sync', socialClerkMiddleware, requireStrictClerkSession, async (req, res) => {
    try {
...
app.get('/api/debug/auth-status', async (req, res) => {
    try {
        const debug = {
            ...
            headers: {
                'x-events-auth-ticket': req.headers['x-events-auth-ticket'] ? 'Ticket present' : 'No ticket',
        ...
        if (parsedTicket) {
            debug.ticket = { userId: parsedTicket.userId, email: parsedTicket.email, message: 'Ticket is valid' };
        } else {
        ...
        return res.json({
            success: true,
            debug,
            recommendation: !parsedTicket ? 'Please ensure you are logged in and passing Unity identity headers/cookie or events ticket.' : 'Authentication check passed',
        });
    } catch (error) {
...
app.get('/api/safe-rooms/trusted-example', requireUnityUser, async (req, res) => {
    if (!req.user.trusted) {
        return res.status(403).json({ error: 'Access denied', message: 'Trusted users only.' });
    }

    return res.json({ success: true, message: 'Welcome to a trusted safe room.' });
});
...
function requireAdmin(req, res, next) {
    ...
    if (!adminEmail) {
        console.error('❌ ADMIN_EMAIL not configured. Set ADMIN_EMAIL or ENCRYPTED_ADMIN_EMAIL + ENCRYPTION_KEY env vars.');
        return res.status(500).json({ 
    ...
    if (!userEmail) {
        console.warn('⚠️ Admin access denied: no x-user-email header or req.user.email');
    ...
    console.warn(`⚠️ Admin access denied for ${userEmail} (expected ${adminEmail})`);
    return res.status(403).json({ 
        error: 'Admin access required'
    });
}
...
function requireAdminOrTherapist(req, res, next) {
    ...
    console.log(`[EVENTS] Permission denied: User role is '${userRole}', requires admin or therapist`);
    return res.status(403).json({ 
        error: 'Permission denied', 
        message: 'Only admins and therapists can create events. Your role is: ' + userRole
    });
}
...
function isTherapistRequest(req) {
    return (req.headers['x-role'] || '').toString().toLowerCase().trim() === 'therapist';
}
...
const normalizeSupportLifecycleStatus = (value, fallback = 'new') => {
    ...
    if (raw === 'in_progress' || raw === 'in-progress' || raw === 'started' || raw === 'active' || raw === 'ongoing') return 'in_progress';
    if (raw === 'ended' || raw === 'closed' || raw === 'done' || raw === 'resolved' || raw === 'completed') return 'ended';
    return fallback;
};
...
const parseSessionStartAtMs = (session) => {
    ...
    const fallback = session?.start_time ? new Date(session.start_time).getTime() : NaN;
    return Number.isNaN(fallback) ? null : fallback;
};

const getJoinStateForSession = (session, nowMs = Date.now()) => {
    const startAtMs = parseSessionStartAtMs(session);
    const normalizedStatus = normalizeSupportLifecycleStatus(session?.status, 'new');

    if (!startAtMs) {
        return {
            joinEnabled: normalizedStatus === 'live' || normalizedStatus === 'in_progress',
            minutesToStart: null,
            startsAt: null,
...
const emitSupportSessionReminder = async (session, minutesLeft) => {
    ...
    io.emit('support_session_reminder', {
        ...
        callMode: String(session.call_mode || 'voice').toLowerCase() === 'video' ? 'video' : 'voice',
        minutesLeft,
        ...
        for (const session of (rows || [])) {
            ...
            if (joinState.minutesToStart !== null) {
                for (const marker of SUPPORT_REMINDER_MINUTES) {
                    if (joinState.minutesToStart <= marker && joinState.minutesToStart > marker - 1) {
...
app.delete('/api/support/sessions/:id', async (req, res) => {
    try {
        ...
        if (!adminRequest && !therapistRequest) {
            return res.status(403).json({ success: false, error: 'Therapist or admin access required' });
        }
        ...
        if (!adminRequest) {
            const therapistIdFromHeader = await resolveTherapistIdFromHeaders(req.headers);
            if (!therapistIdFromHeader || Number(currentSession.therapist_id) !== therapistIdFromHeader) {
        ...
        return res.status(500).json({ success: false, error: 'Failed to delete support session' });
    }
});
...
const loadBuddieEmpathyBank = (candidatePath) => {
    try {
        const resolvedPath = resolveExistingDataPath([candidatePath, ...BUDDIE_EMPATHY_BANK_FALLBACK_PATHS]);
        if (!resolvedPath) {
            const attempted = Array.from(new Set([candidatePath, ...BUDDIE_EMPATHY_BANK_FALLBACK_PATHS].filter(Boolean)))
        ...
        const normalized = Object.entries(parsed).reduce((acc, [emotion, entries]) => {
            ...
            const cleanEntries = entries
                ...
                .filter((entry) => !BUDDIE_UNSAFE_STYLE_PATTERNS.some((pattern) => pattern.test(entry)));

            if (cleanEntries.length) acc[cleanEmotion] = Array.from(new Set(cleanEntries));
            return acc;
        }, {});

        const total = Object.values(normalized).reduce((sum, items) => sum + items.length, 0);
        console.log(`✅ Loaded empathy bank from ${resolvedPath}: ${Object.keys(normalized).length} emotions, ${total} responses`);
        return normalized;
    } catch (error) {
...
const buildStyleExamplesFromEmpathyBank = (bank = {}) => {
    ...
    for (const [emotionKey, responses] of Object.entries(bank)) {
        ...
        const seedPrompt = emotionPromptMap[(emotionKey || '').toLowerCase()] || `I'm feeling ${emotionKey} today.`;
    ...
    return dedupeBuddieExamples(examples).filter(isSafeStyleExample).slice(0, 500);
};
...
const getEmpathyBankFallback = (emotionLabel, vibeLabel, memoryKey) => {
    ...
    const recent = new Set((bucket?.responses || []).slice(-6).map(item => normalizeText(item)));
...
const getDatasetGroundedFallback = (userMessage, emotionLabel, vibeLabel, memoryKey) => {
    ...
    const bucket = conversationMemory.get(memoryKey);
    const recent = new Set((bucket?.responses || []).slice(-8).map(item => normalizeText(item)));
    ...
    const ranked = buddieDialogExamples
        .map((example) => {
            ...
            const questionBoost = hasQuestionSignal(cleanedMessage) && hasQuestionSignal(example.user) ? 1 : 0;
            const emotionValue = (example.emotion || '').toLowerCase();
            const emotionBoost = normalizedEmotion && emotionValue.includes(normalizedEmotion) ? 1 : 0;
...
const resolveBuddieHFDatasets = () => {
    const parsed = BUDDIE_HF_DATASETS
        .map(parseDatasetSpec)
...
const mapHFRowToBuddieExample = (row, datasetName = '') => {
    ...
    if (datasetLower === 'facebook/empathetic_dialogues') {
        ...
        const user = toSafeText(row?.prompt || row?.context || row?.situation || row?.Context || row?.input);
        const buddie = toSafeText(row?.utterance || row?.response || row?.Response || row?.output);
...
const fetchHfDatasetSplits = async (datasetName) => {
    ...
    const response = await fetch(`https://datasets-server.huggingface.co/splits?dataset=${encoded}`);
    if (!response.ok) {
...
const loadBuddieExamplesFromHFDataset = async () => {
    ...
    try {
        ...
        const deduped = dedupeBuddieExamples(allExamples).slice(0, BUDDIE_HF_ROWS_LIMIT);
        if (deduped.length > 0) {
            console.log(`✅ Loaded ${deduped.length} HF examples for Buddie calibration`);
        }
        return deduped;
    } catch (error) {
        console.log(`ℹ️ HF dataset hydration skipped (optional enrichment, Buddie uses local datasets)`);
        return [];
    }
};
...
const loadAllBuddieStyleExamples = () => {
    ...
    const counseling = loadBuddieDialogExamples(BUDDIE_COUNSELING_DATA_PATH, 'MentalHealthCounseling');
        const livesiyam = loadBuddieDialogExamples(BUDDIE_LIVESIYAM_DATA_PATH, 'Livesiyam5000');
    const bankForFallback = loadBuddieEmpathyBank(BUDDIE_EMPATHY_BANK_PATH);
    const empathyBankExamples = buildStyleExamplesFromEmpathyBank(bankForFallback);
    const merged = [...dailyDialog, ...counseling, ...livesiyam, ...empathyBankExamples];

    if (!merged.length) {
        console.warn('⚠️ No conversational calibration datasets loaded. Buddie style examples are empty.');
...
const hydrateBuddieExamplesFromHF = async () => {
    ...
    buddieDialogExamples = dedupeBuddieExamples([...buddieDialogExamples, ...hfExamples]);
    console.log(`✅ Buddie calibration enriched with ${hfExamples.length} HF examples`);
};
...
const getMemoryBucket = (memoryKey) => {
    if (!conversationMemory.has(memoryKey)) {
        conversationMemory.set(memoryKey, { messages: [], responses: [], emotions: [], summary: '', summarizedCount: 0 });
    }
...
const extractFavoriteTopics = (text) => {
    ...
    const topicMap = {
        ...
        relationship: ['relationship', 'boyfriend', 'girlfriend', 'partner', 'breakup'],
...
const updateProfileFromInteraction = (memoryKey, userMessage, emotionLabel, toneInstruction) => {
    const profile = getProfile(memoryKey);
    const topics = extractFavoriteTopics(userMessage);
    const favoriteTopicSet = new Set([...(profile.favoriteTopics || []), ...topics]);
...
const rememberConversation = (memoryKey, userMessage, botResponse, emotionLabel) => {
...
const hasRecentlySaid = (memoryKey, candidate) => {
    ...
    return bucket.responses.some(reply => normalizeText(reply) === normalizedCandidate);
};
...
const getToneByEmotion = (emotionLabel, vibeLabel) => {
    ...
    if (normalizedEmotion === 'sadness') return 'gentle, supportive, and empathetic';
    ...
    if (vibeLabel === VIBE_LABELS.LOW) return 'gentle, supportive, and empathetic';
...
const getEmotionFallback = (emotionLabel, vibeLabel, memoryKey, userMessage = '') => {
    const datasetReply = getDatasetGroundedFallback(userMessage, emotionLabel, vibeLabel, memoryKey);
    if (datasetReply) return datasetReply;

    const empathyBankReply = getEmpathyBankFallback(emotionLabel, vibeLabel, memoryKey);
    if (empathyBankReply) return empathyBankReply;

    return null;
};

const getAdaptiveFallback = ({ kenyanDetection, emotionLabel, vibeLabel, memoryKey, userMessage }) => {
...
async function tryGemini(message) {
    ...
    try {
        ...
        const result = await model.generateContent({
            ...
            generationConfig: {
                maxOutputTokens: BUDDIE_REPLY_MAX_TOKENS,
                temperature: 0.7,
            },
        });
...
const shouldSuggestProfessionalSupport = ({ distressLevel, llmAnalysis, cleanedMessage = '' }) => {
    ...
    const sensitivePattern = /(self harm|hurt myself|hopeless|i can\'t cope|panic attacks?|trauma|abuse|suicid)/i;
...
const summarizeContext = async (memoryKey) => {
    ...
    if (bucket.summary && bucket.summarizedCount === cutoff) return bucket.summary;
    ...
    const contextText = oldMessages
        .map((message, index) => `User: ${message}\nUnity: ${oldResponses[index] || ''}`)
        .join('\n');

    const summaryPrompt = `Summarize this prior chat in 3-5 short sentences. Keep emotional patterns, recurring worries, and wins.\n\n${contextText}`;
    const summaryInstruction = 'You are a concise memory summarizer for a supportive mental-health friend. Return plain text only.';
...
const resolveMemoryKey = (req, bodyUserId) => {
    ...
    const emailHeader = (req.headers['x-user-email'] || '').toString().trim().toLowerCase();
...
const mapEmotionToVibe = (emotionLabel) => {
    ...
    if (['joy', 'love', 'optimism', 'gratitude', 'amusement'].includes(value)) return VIBE_LABELS.POSITIVE;
    if (['sadness', 'grief', 'disappointment', 'loneliness'].includes(value)) return VIBE_LABELS.LOW;
    if (['anger', 'annoyance', 'frustration', 'fear', 'anxiety', 'nervousness'].includes(value)) return VIBE_LABELS.STRESSED;
...
const detectEmotion = async (userText) => {
    ...
    const { signal, cancel } = createTimeoutSignal(Math.min(AI_TIMEOUT_MS, 12000));
    try {
        const response = await fetch(`https://api-inference.huggingface.co/models/${HF_EMOTION_MODEL}`, {
            method: 'POST',
            headers: {
...
const normalizeLlmMessageAnalysis = (raw) => {
    ...
    const allowedIntent = new Set(['greeting', 'small_talk', 'support_request', 'venting', 'advice_request', 'gratitude', 'avoidant', 'crisis']);
    const allowedUrgency = new Set(['low', 'medium', 'high']);

    const parsedConfidence = Number(source.confidence);
    const confidence = Number.isFinite(parsedConfidence) ? Math.max(0, Math.min(1, parsedConfidence)) : DEFAULT_LLM_MESSAGE_ANALYSIS.confidence;

    return {
        intent: allowedIntent.has((source.intent || '').toString()) ? source.intent : DEFAULT_LLM_MESSAGE_ANALYSIS.intent,
        emotionLabel: toSafeText(source.emotionLabel || DEFAULT_LLM_MESSAGE_ANALYSIS.emotionLabel).toLowerCase() || DEFAULT_LLM_MESSAGE_ANALYSIS.emotionLabel,
        urgency: allowedUrgency.has((source.urgency || '').toString()) ? source.urgency : DEFAULT_LLM_MESSAGE_ANALYSIS.urgency,
        confidence,
        recommendedTone: toSafeText(source.recommendedTone || DEFAULT_LLM_MESSAGE_ANALYSIS.recommendedTone),
        responseGoal: toSafeText(source.responseGoal || DEFAULT_LLM_MESSAGE_ANALYSIS.responseGoal),
        followUpStyle: toSafeText(source.followUpStyle || DEFAULT_LLM_MESSAGE_ANALYSIS.followUpStyle),
...
const analyzeMessageWithLLM = async ({ message, mood, note, intensity, history, memoryContext, contextSummary, distressLevel, fallbackEmotion, fallbackVibe }) => {
    ...
    const prompt = [
        ...
        recentUserHistory ? `Recent history:\n${recentUserHistory}` : 'Recent history: none',
        '',
        'Output JSON schema:',
        '{',
    ...
    const instruction = 'You are an emotionally intelligent message analyzer for a supportive friend chatbot. Be precise and safety-first.';

    try {
        const analysis = await callAI(prompt, instruction, { json: true, retries: 1 });
...
const VIBE_KEYWORDS = {
    positive: ['happy', 'good', 'great', 'fine', 'poa', 'sawa', 'freshi', 'nice', 'amazing', 'better', '😄', '✨', '😁', '😊'],
    neutral: ['hi', 'hello', 'hey', 'sasa', 'wassup', 'sup', 'uko poa', 'not bad', 'mambo', 'niaje'],
    low: ['sad', 'tired', 'lonely', 'hurt', 'down', 'niko down', 'sijiskii poa', 'empty', 'drained', 'hopeless', '😔', '😞', '💔'],
    stressed: ['stressed', 'angry', 'frustrated', 'annoyed', 'wawa', 'too much', 'overwhelmed', 'can\'t do this', 'siwezi', 'nimechoka', '😤', '😡'],
    avoidant: ['i dont want support', 'i don\'t want support', 'leave me', 'stop', 'go away', 'not now', 'sitaki', 'acha']
};

const detectUserVibe = ({ message, mood, history }) => {
    const text = `${toSafeText(message)} ${(mood || '').toString()}`.toLowerCase();
    const recentUserLine = Array.isArray(history)
        ? history
            .map(item => (item?.role === 'user' ? toSafeText(item?.text).toLowerCase() : ''))
            .filter(Boolean)
            .slice(-1)[0] || ''
        : '';
    ...
    if (/\b(really bad|terrible|done|fed up)\b/i.test(combined)) score.stressed += 1;
...
const summarizeEmotionTrend = (history) => {
    if (!Array.isArray(history)) return { repeatedLow: false, repeatedStress: false, summary: 'No strong emotional trend detected yet.' };
    ...
    if (!userMessages.length) {
        return { repeatedLow: false, repeatedStress: false, summary: 'No strong emotional trend detected yet.' };
    }
    ...
    if (repeatedLow) {
        ...
        summary = 'User has shown repeated stress/frustration across recent messages.';
    }
...
const buildBuddieUserPrompt = ({ message, mood, note, intensity, history, vibeProfile, trendProfile, distressLevel, memoryContext, emotionLabel, contextSummary, toneInstruction, profileContext, semanticMemory, kenyanSlangContext, llmAnalysis }) => {
    ...
    if (memoryContext) {
        chunks.push('Short-term memory context:', trimPromptChunk(memoryContext, 700));
    }

    if (profileContext) {
        chunks.push(`User profile context: ${trimPromptChunk(profileContext, 260)}`);
    }

    if (semanticMemory) {
        chunks.push(`Relevant past topics: ${trimPromptChunk(semanticMemory, 320)}`);
    }

    if (kenyanSlangContext) {
        chunks.push(`Kenyan slang interpretation: ${trimPromptChunk(kenyanSlangContext, 220)}`);
    }
    ...
    if (trendProfile?.repeatedLow || trendProfile?.repeatedStress) {
        chunks.push('Gently acknowledge patterns over time (e.g., "I’ve noticed this has been heavy for a few days"). Keep it supportive, not clinical.');
    }

    if (distressLevel === 'moderate') {
        chunks.push('User may be emotionally overloaded. Prioritize grounding and emotional safety before suggestions.');
    }
...
const selectDialogExamplesForPrompt = (userPrompt, vibeProfile, count = BUDDIE_DIALOG_FEWSHOT_COUNT) => {
    ...
    const ranked = buddieDialogExamples
        .map(example => {
            ...
            const emotionValue = (example.emotion || '').toLowerCase();
            const vibeMatchBoost = vibeProfile?.label && emotionValue.includes(vibeProfile.label) ? 1 : 0;
...
try {
    if (aiConfig.gemini.key && !aiConfig.gemini.key.startsWith('your_')) {
        genAI = new GoogleGenerativeAI(aiConfig.gemini.key);
        console.log(`✨ Gemini initialized (Model: ${aiConfig.gemini.model})`);
    } else {
        console.warn('⚠️ Gemini API key is missing or placeholder.');
    }
} catch (error) {
...
app.use(async (req, res, next) => {
    try {
        ...
        if (normalizedPath === '/events' || normalizedPath.startsWith('/private/')) {
            const destinationPath = normalizedPath === '/events' ? '/' : normalizedPath;
        ...
        try {
            ...
            console.warn('Events slug lookup failed, falling back to canonical redirect:', slugLookupError?.message || slugLookupError);
            const target = `${resolveCanonicalEventsBase(req)}/${slug}${req.url.includes('?') ? `?${req.url.split('?')[1]}` : ''}`;
            return res.redirect(302, target);
        }
    } catch (error) {
...
app.post('/api/support/therapist-login', async (req, res) => {
    try {
        ...
        if (!['approved', 'active'].includes((therapist.status || '').toLowerCase())) {
            return res.status(403).json({ error: 'Not approved', message: 'Therapist account is pending admin approval.' });
        }

        if (!therapist.password_hash) {
            return res.status(403).json({ error: 'Password not set', message: 'Your therapist password has not been set by admin.' });
        }
        ...
        return res.status(500).json({ error: 'Server error', message: 'Something went wrong. Please try again.' });
    }
});
...
app.get('/api/auth/unity-user', async (req, res) => {
    try {
        ...
        if (!payload?.userId) {
            return res.status(401).json({ success: false, error: 'No shared session' });
        }
        ...
        return res.status(500).json({ success: false, error: 'Failed to bootstrap shared user' });
    }
});
...
app.put('/api/profile', requireUnityUser, async (req, res) => {
    try {
        ...
        const allowed = ['name', 'display_name', 'avatar', 'profile_image', 'ageRange', 'bio', 'isAnonymous', 'emergency_contact', 'notificationPrefs', 'goals', 'profileVisibility'];
        const updates = [];
        const values = [];
        allowed.forEach((key) => {
    ...
    if (groq) {
        ...
        for (let i = 0; i < retries; i++) {
            try {
                ...
                const isRetryable = isRateLimit || status === 503 || status === 504;
                if (isRetryable && i < retries - 1) {
                    console.warn(`⏳ Groq rate limit/unavailable (Attempt ${i + 1}). Retrying in ${currentDelay}ms...`);
                    await sleep(currentDelay);
                    currentDelay *= 2;
                } else {
...
io.on('connection', (socket) => {
    ...
    socket.on('join_event', async (payload) => {
        ...
        try {
            ...
            if (!rows?.length) {
                socket.emit('event_access_denied', { reason: 'RSVP YES is required before joining event chat.' });
                return;
            }
            ...
            socket.emit('event_access_denied', { reason: 'Unable to join event room right now.' });
        }
    });
    ...
    socket.on('event_message', async (payload) => {
        ...
        if (!eventId || !userId || !message) {
            socket.emit('event_message_rejected', { reason: 'Invalid event message payload.' });
            return;
        }
        ...
        try {
            ...
            if (!rsvpRows?.length) {
                socket.emit('event_message_rejected', { reason: 'RSVP YES is required before chatting.' });
                return;
            }
            ...
            socket.emit('event_message_rejected', { reason: 'Unable to send message right now.' });
        }
    });
    ...
    socket.on('send_message', async (data) => {
        ...
        const userId = (typeof data?.userId === 'number' && Number.isFinite(data.userId)) ? data.userId : null;
        const content = typeof data?.content === 'string' ? data.content.trim() : '';
        const isAnonymous = Boolean(data?.isAnonymous);

        if (!roomId || !content) {
            socket.emit('message_rejected', { reason: 'Message is empty or room is invalid.' });
            return;
        }
        ...
        try {
            ...
            console.error('Moderation failed, defaulting to safe:', moderationError?.message || moderationError);
        }
        if (!moderation.safe) {
    ...
    socket.on('join_support_session', async (sessionId) => {
        ...
        try {
            ...
            if (!['confirmed', 'live', 'accepted', 'in_progress'].includes(normalizedStatus)) {
                return;
            }
    ...
    socket.on('support_send_message', async (data) => {
        ...
        const senderName = toSafeText(data?.senderName) || (senderRole === 'therapist' ? 'Therapist' : 'User');
        const attachmentName = toSafeText(data?.attachmentName) || null;

        if (!sessionId || !content) {
            socket.emit('support_message_rejected', { reason: 'Message is empty or session is invalid.' });
            return;
        }
...
const buildEventPublicLink = (req, slug) => {
    const configuredBase = (process.env.EVENTS_APP_BASE_URL || process.env.APPSETTING_EVENTS_APP_BASE_URL || '').toString().trim();
    if (configuredBase) {
    ...
    const isUnityDomain = bareHost === 'unitywithin.app' || bareHost.endsWith('.unitywithin.app');
    const canonicalProtocol = isUnityDomain ? 'https' : protocol;
    const canonicalHost = isLocal
        ? requestHost
        : (isUnityDomain ? 'unitywithin.app/events' : requestHost || 'unitywithin.app/events');
...
const MAIN_APP_RESERVED_PATHS = new Set([
    'dashboard', 'toolkit', 'chat', 'community', 'journal', 'breathe', 'education', 'admin',
    'namethefeeling', 'selfcompassion', 'values', 'bodyscan', 'safespace', 'reframer', 'profile',
    'support', 'landingpage', 'signup', 'sign-up', 'login', 'sign-in', 'sso-callback',
    'therapist-login', 'therapist-portal', 'therapist-invite', 'forgot-password', 'reset-password',
    'why-unity', 'privacy', 'api'
]);
...
app.get('/api/push/public-key', (req, res) => {
    const publicKey = process.env.VITE_VAPID_PUBLIC_KEY || process.env.VAPID_PUBLIC_KEY || process.env.APPSETTING_VAPID_PUBLIC_KEY;
    if (!publicKey) {
...
app.post('/api/admin/invite-therapist', requireAdmin, async (req, res) => {
    try {
        ...
        const token = crypto.randomBytes(32).toString('hex');
        const appBaseUrl = (process.env.APP_BASE_URL || process.env.RESET_PASSWORD_BASE_URL || 'https://unitywithin.app').replace(/\/$/, '');
        ...
        const emailErrorMessage = emailSent ? null : (emailResult.error || 'Email delivery failed');
        ...
        const whatsappUrl = sanitizedPhone ? `https://wa.me/${sanitizedPhone}?text=${whatsappMessage}` : null;
        ...
        return res.status(500).json({ success: false, error: 'Failed to create therapist invite' });
    }
});
...
app.get('/api/invite/:token', async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, error: 'Failed to verify invite' });
    }
});
...
app.post('/api/invite/complete', async (req, res) => {
    try {
        ...
        const languages = (req.body?.languages || 'English, Swahili').toString().trim();
        ...
        if (normalizeTherapistStatus(invite.status) !== 'pending') {
            return res.status(400).json({ success: false, error: 'Invite has already been used' });
        }
        ...
        return res.status(500).json({ success: false, error: 'Failed to complete invite onboarding' });
    }
});
...
app.get('/api/support/therapists', async (req, res) => {
    try {
        ...
        if (specialization) {
            where.push('specialization ILIKE ?');
            params.push(`%${specialization}%`);
        }

        if (language) {
            where.push('languages ILIKE ?');
            params.push(`%${language}%`);
        }

        if (availability) {
            where.push('availability ILIKE ?');
        ...
        return res.status(500).json({ success: false, error: 'Failed to fetch therapists' });
    }
});
...
app.get('/api/support/therapists/:id', async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, error: 'Failed to fetch therapist profile' });
    }
});
...
app.get('/api/support/therapist-context', async (req, res) => {
    try {
        ...
        if (!therapistId) {
            return res.status(404).json({ success: false, error: 'Therapist profile not found for current account' });
        }
        ...
        return res.status(500).json({ success: false, error: 'Failed to resolve therapist context' });
    }
});
    ...
    try {
        ...
        return res.json({
            ...
            data: {
                ...
                sessionType: String(session.call_mode || 'voice').toLowerCase() === 'video' ? 'video' : 'audio',
        ...
        return res.status(500).json({ success: false, error: 'Failed to fetch active support session' });
    }
});
...
app.post('/api/sessions/book', async (req, res) => {
    try {
        ...
        const preferredTimeFrame = (req.body?.preferredTimeFrame || '').toString().trim();
        ...
        const issueDescription = (req.body?.issueDescription || '').toString().trim();

        if (!therapistId || !userId || !date || !time || !['video', 'voice'].includes(callMode)) {
            return res.status(400).json({ success: false, error: 'therapistId, userId, date, time, and type(video|voice) are required' });
        }
        ...
        if (activeRows?.length) {
            return res.status(409).json({
                success: false,
                error: 'You already have an active request/session with this therapist. Finish it before booking another one.'
            });
        }
...
app.patch('/api/sessions/:id', async (req, res) => {
    try {
        ...
        if (Number(currentSession.user_id) !== userId) {
            return res.status(403).json({ success: false, error: 'You can only edit your own sessions' });
        }

        if (!['new', 'pending'].includes(String(currentSession.status).toLowerCase())) {
            return res.status(400).json({ success: false, error: 'Can only edit pending sessions' });
        }
...
app.get('/api/therapist/sessions', async (req, res) => {
    try {
        const therapistId = await resolveTherapistIdFromHeaders(req.headers);
        if (!therapistId) {
            return res.status(400).json({ success: false, error: 'Therapist identity not found' });
        }
        ...
        return res.status(500).json({ success: false, error: 'Failed to fetch therapist sessions' });
    }
});
...
app.get('/api/support/sessions/:id', async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, error: 'Failed to fetch session details' });
    }
});
...
app.post('/api/sessions/:id/accept', async (req, res) => {
    try {
        ...
        return res.json({ success: true, status: 'confirmed', callMode, scheduledDate, scheduledTime });
    } catch (error) {
...
app.post('/api/sessions/:id/reject', async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, error: 'Failed to reject session' });
    }
});
...
const normalizeSupportStatus = (value, fallback = 'new') => {
    ...
    if (raw === 'live') return 'live';
    if (raw === 'rejected') return 'rejected';
    if (raw === 'in_progress' || raw === 'in-progress' || raw === 'started' || raw === 'active') return 'in_progress';
    if (raw === 'ongoing') return 'in_progress';
    if (raw === 'ended' || raw === 'closed' || raw === 'done' || raw === 'resolved') return 'ended';
...
const normalizeSupportPriority = (value, fallback = 'normal') => {
    ...
    if (raw === 'normal' || raw === 'medium' || raw === 'default') return 'normal';
...
app.get('/api/support/portal/sessions', async (req, res) => {
    try {
        ...
        if (!adminRequest) {
            if (!therapistIdFromHeader) {
                return res.status(400).json({ success: false, error: 'Missing therapist id' });
            }
        ...
        return res.status(500).json({ success: false, error: 'Failed to load therapist portal data' });
    }
});
...
app.patch('/api/support/sessions/:id/assign', async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, error: 'Failed to assign session therapist' });
    }
});
...
app.patch('/api/support/sessions/:id/state', async (req, res) => {
    try {
        ...
        const therapistIdFromHeader = await resolveTherapistIdFromHeaders(req.headers);
        if (!adminRequest && Number(currentSession.therapist_id) !== therapistIdFromHeader) {
            return res.status(403).json({ success: false, error: 'You can only update your own sessions' });
        }

        const requestedStatus = typeof req.body?.status === 'string' ? req.body.status : undefined;
        const requestedPriority = typeof req.body?.priority === 'string' ? req.body.priority : undefined;

        if (!requestedStatus && !requestedPriority) {
            return res.status(400).json({ success: false, error: 'status or priority is required' });
        }

        const nextStatus = normalizeSupportStatus(requestedStatus, normalizeSupportStatus(currentSession.status));
        let nextPriority = normalizeSupportPriority(requestedPriority, normalizeSupportPriority(currentSession.priority));
        ...
        io.emit('support_session_status_changed', {
            sessionId,
            status: nextStatus === 'in_progress' ? 'ongoing' : (nextStatus === 'ended' ? 'completed' : nextStatus),
            therapistId: Number(currentSession.therapist_id) || null,
...
app.patch('/api/notifications/:id/read', async (req, res) => {
    try {
        ...
        if (!notificationId || !userId) {
            return res.status(400).json({ success: false, error: 'Valid notification id and userId are required' });
        }
...
// Admin: Delete RSVP
app.delete('/api/admin/events/:eventId/rsvps/:rsvpId', requireAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/therapists', requireAdmin, async (req, res) => {
    try {
        ...
        if (!name || !specialization) {
            return res.status(400).json({ success: false, error: 'name and specialization are required' });
        }

        const hashedTherapistPassword = password ? await bcrypt.hash(password, 10) : null;
...
app.patch('/api/admin/therapists/:id/approve', requireAdmin, async (req, res) => {
    try {
        const therapistId = Number(req.params.id);
...
app.post('/api/chat/messages', requireUnityUser, async (req, res) => {
    try {
        ...
        console.log(`📩 Received message from User ${req.user.id}: "${content}" (ReplyTo: ${replyToId})`);
...
app.post('/api/moods', async (req, res) => {
    try {
        ...
        if (!userId || !mood) {
            return res.status(400).json({ error: 'Missing required field(s): userId and mood are required' });
        }
...
app.get('/api/events', async (req, res) => {
    try {
        ...
        const mappedEvents = (events || []).map((eventRow) => {
            ...
            const capacity = eventRow.capacity !== null && eventRow.capacity !== undefined ? Number(eventRow.capacity) : null;
...
app.get('/api/events/private/:token', async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, error: 'Failed to load private event' });
    }
});
...
app.get('/api/events/:eventId/calendar', async (req, res) => {
    try {
        ...
        const end = eventRow.end_date ? new Date(eventRow.end_date) : new Date(start.getTime() + 60 * 60 * 1000);
        const toIcsDate = (value) => value.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}Z$/, 'Z');
        const description = String(eventRow.description || '').replace(/\n/g, '\\n');

        res.setHeader('Content-Type', 'text/calendar; charset=utf-8');
        res.setHeader('Content-Disposition', `attachment; filename="${eventRow.slug || `event-${eventId}`}.ics"`);
        return res.send([
            'BEGIN:VCALENDAR',
            'VERSION:2.0',
        ...
        return res.status(500).json({ success: false, error: 'Failed to generate calendar file' });
    }
});

app.get('/api/events/recommendations', async (req, res) => {
    try {
        const category = (req.query.category || req.query.interest || '').toString().trim().toLowerCase();
...
app.get('/api/events/:slug', async (req, res) => {
    try {
        ...
        const capacity = row.capacity !== null && row.capacity !== undefined ? Number(row.capacity) : null;
        const spotsLeft = capacity ? Math.max(capacity - attendeesCount, 0) : null;
...
app.post('/api/events', requireEventsRsvpIdentity, requireAdminOrTherapist, async (req, res) => {
    try {
        const {
            title,
            tagline,
        ...
        if (thumbnailUrl) {
            console.log(`[Events] Thumbnail size: ${(thumbnailUrl.length / 1024).toFixed(2)}KB`);
        }
        if (description) {
            console.log(`[Events] Description size: ${(description.length / 1024).toFixed(2)}KB`);
        }
        ...
        console.log(`[Events] Creation successful: ID ${result.insertId}, slug "${eventSlug}"`);
        return res.status(201).json({ success: true, id: result.insertId, slug: eventSlug });
    } catch (error) {
        ...
        if (error.code === 'ER_DUP_ENTRY' || error.message?.includes('slug already exists')) {
            return res.status(409).json({ 
                success: false, 
        ...
        if (error.code === 'ER_DATA_TOO_LONG') {
            return res.status(400).json({ 
                ...
                message: 'One or more fields contain too much data. Please reduce the size of images, descriptions, or other large content.'
            });
        }
...
app.post('/api/events/:eventId/media', requireUnityUser, requireAdmin, async (req, res) => {
    try {
        const eventId = Number(req.params.eventId);
        ...
        return res.status(500).json({ success: false, error: 'Failed to update event media' });
    }
});

app.post('/api/events/:eventId/invites', requireEventsRsvpIdentity, requireAdminOrTherapist, async (req, res) => {
    try {
        const eventId = Number(req.params.eventId);
        const inviteEmails = Array.isArray(req.body?.emails) ? req.body.emails : [];
        const expiresInHours = Math.max(1, Number(req.body?.expiresInHours || 72));
        ...
        if (!eventRows[0].is_private) {
            return res.status(400).json({ success: false, error: 'Invites are only available for private events' });
        }
        ...
        return res.status(500).json({ success: false, error: 'Failed to create event invites' });
    }
});

app.post('/api/events/:eventId/rsvp', requireEventsRsvpIdentity, async (req, res) => {
    try {
        const eventId = Number(req.params.eventId);
        const status = (req.body?.status || '').toString().toLowerCase();
        const paid = Boolean(req.body?.paid);
        const redirectSource = (req.body?.redirectSource || '').toString().slice(0, 80);
        const redirectContextRaw = req.body?.redirectContext;
        const redirectContext = redirectContextRaw ? JSON.stringify(redirectContextRaw).slice(0, 2000) : null;
        ...
        if (!userId || userId === 0) {
            ...
            try {
                ...
                if (!userId) {
                    console.error('Failed to create/fetch user for RSVP:', userCreateError);
                    return res.status(500).json({ success: false, error: 'Failed to create user record' });
                }
            }
        }
        ...
        if (existingRsvpRows?.length) {
            ...
            if (existingStatus === 'yes') {
                ...
                return res.status(409).json({ success: false, error: 'You have already RSVP\'d to this event. Your seat is reserved.' });
            }
            ...
            if (status === 'yes') {
                ...
                return res.status(409).json({ success: false, error: 'Your RSVP status cannot be changed once confirmed.' });
            }
        } else {
            await pool.query(
        ...
        const capacity = eventRows[0].capacity !== null && eventRows[0].capacity !== undefined
        ...
        return res.status(500).json({ success: false, error: 'Failed to save RSVP' });
    }
});
...
app.get('/api/events/:eventId/messages', requireUnityUser, async (req, res) => {
    try {
        ...
        if (!rsvpRows?.length) {
            return res.status(403).json({ success: false, error: 'RSVP yes required to access chat' });
        }
        ...
        return res.status(500).json({ success: false, error: 'Failed to fetch event messages' });
    }
});
...
app.post('/api/buddie/debug-vibe', async (req, res) => {
    try {
        ...
        const kenyanDetection = await detectEmotionWithKenyanLayer(cleanedMessage, detectEmotion);
        const emotionLabel = kenyanDetection.emotionLabel;
        const vibeProfile = detectUserVibe({ message: cleanedMessage, mood, history });
        ...
        const distressLevel = detectDistressLevel({ message: cleanedMessage, mood, history });
        ...
        const similarMemories = await querySimilarMemory(memoryKey, cleanedMessage, 3);
        const profile = getProfile(memoryKey);
        ...
        return res.json({
            ...
            diagnostics: {
                ...
                contextSummary,
                similarMemories,
                hasRecentRepeatCandidate: cleanedMessage ? hasRecentlySaid(memoryKey, cleanedMessage) : false
            }
        });
    } catch (error) {
        return res.status(500).json({ success: false, error: 'Failed to inspect buddie signals' });
    }
});
...
app.post('/api/buddie/respond', async (req, res) => {
    try {
        ...
        if (checkForCrisis(cleanedMessage)) {
            ...
            return res.json({ success: true, message: crisisResponse, supportSuggestion: true });
        }
        ...
        const similarMemories = await querySimilarMemory(memoryKey, cleanedMessage, 3);
        const semanticMemory = similarMemories.length ? similarMemories.join(' | ') : '';
        ...
        if (distressLevel === 'severe') {
            const crisisResponse = 'I’m really glad you told me this. You matter deeply 🤍 Please reach out right now: in Kenya call 1199 (Red Cross) or +254 722 178 177 (Befrienders Kenya), or contact UNITY WITHIN Support at +254 715 765 561. If you are in immediate danger, call local emergency services now.';
            rememberConversation(memoryKey, cleanedMessage, crisisResponse, emotionLabel || vibeProfile.label);
            return res.json({
                success: true,
        ...
        const aiReply = await buddieAI(userPrompt, emotionLabel, vibeProfile.label, memoryKey);
        ...
        if (aiReply) {
            ...
            rememberConversation(memoryKey, cleanedMessage, safeAiResponse, emotionLabel || vibeProfile.label);
            updateProfileFromInteraction(memoryKey, cleanedMessage, emotionLabel || vibeProfile.label, toneInstruction);
            const supportSuggestion = shouldSuggestProfessionalSupport({ distressLevel, llmAnalysis, cleanedMessage });
            return res.json({
                success: true,
        ...
        const localFallback = getAdaptiveFallback({
            kenyanDetection,
            emotionLabel,
            vibeLabel: vibeProfile.label,
            memoryKey,
        ...
        rememberConversation(memoryKey, cleanedMessage, localFallback, emotionLabel || vibeProfile.label);
        updateProfileFromInteraction(memoryKey, cleanedMessage, emotionLabel || vibeProfile.label, toneInstruction);
        return res.json({
            success: true,
            message: localFallback,
            supportSuggestion: shouldSuggestProfessionalSupport({ distressLevel, llmAnalysis, cleanedMessage })
        });
...
If anxiety is frequent, intense, or interfering with daily life, reaching out to a licensed mental health professional is a strong and healthy next step. You deserve support, and recovery is possible one small step at a time.`,
                sources: [
                    { name: 'Mayo Clinic', url: 'https://www.mayoclinic.org/diseases-conditions/anxiety/symptoms-causes/syc-20350961' },
                    { name: 'Healthline', url: 'https://www.healthline.com/health/anxiety' },
                    { name: 'Verywell Mind', url: 'https://www.verywellmind.com/what-is-anxiety-2795188' }
                ]
            };
        }
        ...
        return {
            ...
            sources: [
                { name: 'Psychology Today', url: `https://www.psychologytoday.com/us/archive?search=${(topicTitle || 'Mental Health').replace(/ /g, '+')}` },
                { name: 'Healthline', url: `https://www.healthline.com/search?q=${(topicTitle || 'Mental Health').replace(/ /g, '+')}` },
                { name: 'VeryWell Mind', url: `https://www.verywellmind.com/search?q=${(topicTitle || 'Mental Health').replace(/ /g, '+')}` }
            ]
        };
    };
    ...
    try {
        ...
        if (data) {
            ...
            return res.json({
                ...
                sources: Array.isArray(data.sources) && data.sources.length > 0 ? data.sources : fallback.sources
            });
        }
...
const resolveStaticDistPath = () => {
    ...
    const validCandidates = uniqueCandidates
        .map((candidatePath) => {
            ...
            const shellFile = shellPriority.find((fileName) => fs.existsSync(path.join(candidatePath, fileName)));
            if (!shellFile) {
                return null;
            }
